...
export function AssessmentSwitcher({ assessmentType, setAssessmentType }) {
  return (
    <div className="grid grid-cols-3 bg-zinc-100 dark:bg-zinc-900 border border-zinc-200/50 dark:border-zinc-800/50 rounded-xl p-1 max-w-lg w-full">
      <button
        type="button"
        onClick={() => setAssessmentType("reflective_journal")}
        className={`py-2 rounded-lg text-[11px] sm:text-xs font-semibold transition-all ${
      ...
      <button
        type="button"
        onClick={() => setAssessmentType("literature_survey")}
        className={`py-2 rounded-lg text-[11px] sm:text-xs font-semibold transition-all ${
